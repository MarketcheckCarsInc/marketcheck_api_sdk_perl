...
#
package marketcheck_api_sdk_perl::FacetsApi;
...
# Facets
# 
# @param string $fields Comma separated list of fields to generate facets for. Supported fields are - year, make, model, trim, exterior_color, interior_color, drivetrain, vehicle_type, car_type, body_style, body_subtype, doors (required)
# @param string $api_key The API Authentication Key. Mandatory with all API calls. (optional)
# @param string $vin VIN as a reference to the type of car for which facets data is to be returned (optional)
...
# @param string $trim Trim of the Car (optional)
{
    ...
    __PACKAGE__->method_documentation->{ 'get_facets' } = { 
    	summary => 'Facets',
        params => $params,
        returns => 'ARRAY[FacetItem]',
        };
}
...
sub get_facets {
    ...
    unless (exists $args{'fields'}) {
      croak("Missing the required parameter 'fields' when calling get_facets");
    }

    # parse inputs
    my $_resource_path = '/facets';
    ...
    my $_response_object = $self->{api_client}->deserialize('ARRAY[FacetItem]', $response);
