...
#
package marketcheck_api_sdk_perl::ApiClient;
...
use marketcheck_api_sdk_perl::Configuration;
...
sub new {
    ...
    my $config;
    if ( $_[0] && ref $_[0] && ref $_[0] eq 'marketcheck_api_sdk_perl::Configuration' ) {
        $config = $_[0];
    } else {
        $config = marketcheck_api_sdk_perl::Configuration->new(@_);
    }
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("marketcheck_api_sdk_perl::Object::$class")->new;
        if (ref $data eq "HASH") {
